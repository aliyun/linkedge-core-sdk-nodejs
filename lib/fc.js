...
class FCClient {
  ...
  invokeFunction(params, callback) {
    ...
    if (functionId === undefined) {
      var serviceName = util.getRequiredParameter(params, 'serviceName');
      if (serviceName === undefined) {
        callback(new Error('Can\'t find "functionId" or "serviceName". Function can be ' +
          'invoked via either "functionId" or "serviceName" and "functionName".'));
        return;
      }
      var functionName = util.getRequiredParameter(params, 'functionName');
      if (functionName === undefined) {
        callback(new Error('Can\'t find "functionId" or "functionName". Function can be ' +
          'invoked via either "functionId" or "serviceName" and "functionName".'));
        return;
      }
      functionId = common.buildArnString('', '', serviceName, functionName);
    }
